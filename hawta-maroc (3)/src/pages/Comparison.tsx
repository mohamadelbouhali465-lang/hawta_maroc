...
import { X, ShoppingCart, Check, Star, CreditCard } from 'lucide-react';
import { Link } from 'react-router-dom';
import { useTranslation, getTranslatedCategory } from '../translations';
import { useNavigate } from 'react-router-dom';

export const Comparison: React.FC = () => {
  const { compareList, removeFromCompare, buyPromotion, language } = useAppContext();
            ...
            <tr>
              ...
              {compareList.map(p => (
                <td key={p.id} className={`p-6 ${isRTL ? 'border-l last:border-l-0' : 'border-r last:border-r-0'}`}>
                  ...
                  <button
                    onClick={() => buyPromotion(p)}
                    className="mt-2 w-full inline-flex items-center justify-center gap-2 bg-green-600 text-white py-2 rounded font-bold hover:bg-green-700 shadow-sm transition-all"
                  >
