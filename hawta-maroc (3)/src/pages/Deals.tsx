...
export const Deals: React.FC = () => {
  ...
  const { searchQuery, promotions, addNotification, language } = useAppContext();
  ...
  const filteredDeals = promotions.filter(p => {
    ...
    <div className={`max-w-7xl mx-auto px-4 py-8 ${isRTL ? 'font-sans' : ''}`} dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="flex flex-col md:flex-row gap-8">
                ...
                {CATEGORIES.map(category => (
                  ...
                  >
                    ...
                    <span className={`text-[10px] text-gray-400 font-normal ${isRTL ? 'mr-auto' : 'ml-auto'}`}>
                      ({promotions.filter(p => category === 'All' || p.category === category).length})
                    </span>
                  </li>
                ))}
              </ul>
        ...
        <main className="flex-grow">
          ...
          <div className="bg-white p-4 rounded-lg alibaba-shadow mb-6 flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="flex items-center border border-gray-200 rounded overflow-hidden">
                <button onClick={() => setViewMode('grid')} className={`p-1.5 border-r border-gray-200 ${viewMode === 'grid' ? 'bg-gray-50 text-primary' : 'hover:bg-gray-50 text-gray-400'}`}><Grid className="w-4 h-4" /></button>
                <button onClick={() => setViewMode('list')} className={`p-1.5 ${viewMode === 'list' ? 'bg-gray-50 text-primary' : 'hover:bg-gray-50 text-gray-400'}`}><ListIcon className="w-4 h-4" /></button>
              </div>
            </div>
          </div>
          ...
          {/* Grid */}
          <div className={viewMode === 'grid' ? 'grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6' : 'grid grid-cols-1 md:grid-cols-2 gap-4'}>
            {filteredDeals.slice((page - 1) * 12, page * 12).map((promo) => (
          ...
          <div className="mt-12 flex justify-center items-center gap-2">
             ...
             <button onClick={() => setPage(1)} className={`w-10 h-10 rounded font-bold text-sm ${page === 1 ? 'bg-primary text-white' : 'border border-gray-300 hover:bg-gray-50'}`}>1</button>
             <button onClick={() => setPage(2)} className={`w-10 h-10 rounded font-bold text-sm ${page === 2 ? 'bg-primary text-white' : 'border border-gray-300 hover:bg-gray-50'}`}>2</button>
             <button onClick={() => setPage(3)} className={`w-10 h-10 rounded font-bold text-sm ${page === 3 ? 'bg-primary text-white' : 'border border-gray-300 hover:bg-gray-50'}`}>3</button>
